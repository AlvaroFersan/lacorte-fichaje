...
const COLUMNAS = [
  ...
  { clave: 'progreso', nombre: 'Subtareas', tipo: 'progreso', visible: 1, ancho: 122, propia: 0, orden: 5 },
  { clave: 'horas', nombre: 'Horas', tipo: 'horas', visible: 1, ancho: 95, propia: 0, orden: 6 },
  { clave: 'bloque', nombre: 'Carpeta', tipo: 'bloque', visible: 0, ancho: 200, propia: 0, orden: 7 },
  { clave: 'proyecto', nombre: 'Proyecto', tipo: 'proyecto', visible: 0, ancho: 185, propia: 0, orden: 8 },
  { clave: 'coments', nombre: 'Comentarios', tipo: 'coments', visible: 0, ancho: 125, propia: 0, orden: 9 },
  { clave: 'c_material', nombre: 'Material', tipo: 'etiqueta', visible: 1, ancho: 160, propia: 1, orden: 10,
    opciones: JSON.stringify([{ v: 'Sin material', c: '#e34948' }, { v: 'Ingestado', c: '#eda100' }, { v: 'Sincronizado', c: '#1baf7a' }]) },
  { clave: 'c_dur', nombre: 'Duración (min)', tipo: 'numero', visible: 1, ancho: 125, propia: 1, orden: 11 },
  { clave: 'c_visto', nombre: 'Visto dirección', tipo: 'casilla', visible: 0, ancho: 140, propia: 1, orden: 12 }
];
...
async function sembrarSiVacia() {
  ...
  const hash = await hashear(CLAVE_DEMO);
  const insU = base.prepare(
    'INSERT INTO usuarios (usuario, nombre, iniciales, rol, activo, alta, hash) VALUES (?, ?, ?, ?, 1, ?, ?)'
  );
  for (const u of USUARIOS) insU.run(u.usuario, u.nombre, u.iniciales, u.rol, u.alta, hash);
  ...
  const insC = base.prepare('INSERT INTO carpetas (proyecto_id, nombre, padre_id) VALUES (?, ?, ?)');
  insC.run(proyId, '00_ASISTENCIA', null);
  const doc = insC.run(proyId, '01_DOCUMENTOS', null).lastInsertRowid;
  for (const nomb of ['01_Documentos', '02_Guion', '03_Planes de Trabajo', '04_Crew List']) {
    insC.run(proyId, nomb, doc);
  }
  insC.run(proyId, '02_ESCENAS', null);
  insC.run(proyId, '03_ENTREGAS', null);
  insC.run(proyId, '04_SALIDAS', null);
